        ...
        signUpBtn.setOnClickListener()
        {
            ...
            { task ->
                ...
                if (task.isSuccessful)
                {
                    ...
                    user["email"] = email
                    user["avatarID"] = 1
                    userDocumentReference.set(user).addOnSuccessListener()
                    {
                        Log.d(TAG, "onSuccess:user profile is created for $userID")
                        Toast.makeText(this, "User Profile is created for $userID", Toast.LENGTH_SHORT).show()
                    }

                    // Create a new document for the Crypto Wallet
                    val userCryptoDocumentReference: DocumentReference = _firestore.collection("usersCryptoBalance").document(userID)
                    userCryptoDocumentReference.set(user).addOnSuccessListener()
                    {
                        Log.d(TAG, "onSuccess:user crypto wallet is created for $userID")
                    }
